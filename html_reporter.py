    ...
    for pc in page_comparisons:
        if pc["status"] == "matched":
            ...
            if r < 1.0 and matched_idx < len(per_page_lines):
                ...
                diff_b64 = base64.b64encode(gzip.compress(_raw, compresslevel=9)).decode('ascii')
...
def pair_card(outcome: FilePairOutcome,
    ...
    # When CSV mode is active, diff key-sorted lines so the diff reflects real
    # value changes rather than row-order differences.
    ...
    if file_csv_comparisons:
        ...
        for rc in file_csv_comparisons:
            ...
            if rc["status"] == "matched":
                ...
                diff_b64 = None
                if cr < 1.0:
                    diff_b64 = _b64gz(_diff_rows_1v1(
                        rc["row_a"].line, rc["row_b"].line,
                        rc["row_a"].line_number, rc["row_b"].line_number,
                        normalize=normalize))
                csv_entries.append([0, key, cr, ca, cd, diff_b64])
...
async function toggleEntryDiff(btn,panelId,entryIdx){
  ...
  const dw=document.createElement('div');dw.className='diff-wrap';dw._diffB64=diffB64;
...
function renderCsvPanel(el,srcId){
  ...
  if(s.removed)hdr+=` · <strong style="color:var(--s-removed)">${s.removed}</strong> removed`;
  hdr+='</div>';
  const tbody=_makePanelScaffold(el,hdr,'<th>Key</th><th>Status</th><th class="num">Similarity</th><th class="num">Changes</th><th></th>');
  _chunkRows(d.rows,tbody,(r,i)=>{
    const tr=document.createElement('tr');
    if(r[0]===0){
      const[,key,ratio,add,ndel,diffB64]=r;
      const css=ratio===1?'s-identical':ratio>=0.85?'s-minor':ratio>=0.60?'s-moderate':'s-significant';
      const badge=ratio===1?'IDENTICAL':ratio>=0.85?'MINOR':ratio>=0.60?'MODERATE':'CHANGED';
      const dcell=diffB64?`<span class="card-toggle" onclick="toggleEntryDiff(this,'${pid}',${i})">▶ diff</span>`:'<span class="card-toggle no-diff">✓</span>';
      tr.className=css;
      tr.innerHTML=`<td class="mono" title="${escHtml(key)}">${escHtml(key)}</td><td><span class="badge ${css}">${badge}</span></td><td class="num">${(ratio*100).toFixed(1)}%</td><td class="num mono">+${add} −${ndel}</td><td>${dcell}</td>`;
    }else if(r[0]===1){tr.className='s-added';tr.innerHTML=`<td class="mono" title="${escHtml(r[1])}">${escHtml(r[1])}</td><td><span class="badge s-added">ADDED</span></td><td class="num">—</td><td class="num mono">—</td><td></td>`;}
    else{tr.className='s-removed';tr.innerHTML=`<td class="mono" title="${escHtml(r[1])}">${escHtml(r[1])}</td><td><span class="badge s-removed">REMOVED</span></td><td class="num">—</td><td class="num mono">—</td><td></td>`;}
    return tr;
  });
  el.appendChild(wrap);
  el.dataset.rendered='1';
